...
class MotusAPIClient:
    ...
    def run_inference(
        ...
        if auto_find_t5_embeddings is not True:
            payload["auto_find_t5_embeddings"] = auto_find_t5_embeddings
        if return_frame_grid:
            payload["return_frame_grid"] = return_frame_grid
    ...
    def run_mock_inference(self, timeout: int = 30) -> Dict[str, Any]:
        response = self.session.post(f"{self.base_url}/inference/mock", timeout=timeout)
        response.raise_for_status()
        return response.json()
...
def test_model_info(client: MotusAPIClient) -> bool:
    ...
    try:
        ...
        print(f"  Checkpoint: {config.get('checkpoint_path')}")
        print(f"  T5 embeddings dir: {config.get('t5_embeddings_dir')}")
        print(f"  Default instruction: {config.get('default_instruction')}")
        print(f"  State dim: {config.get('state_dim')}")
        print(f"  Action dim: {config.get('action_dim')}")
        print(f"  Video size: {config.get('video_height')} x {config.get('video_width')}")
        return True
    except Exception as exc:
...
def test_mock_inference(client: MotusAPIClient) -> bool:
    ...
    try:
        ...
        print(f"Mock inference completed in {elapsed_ms:.2f}ms")
        ...
        print(f"Mock inference failed: {exc}")
        return False
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--test",
        choices=["connectivity", "model_info", "mock", "inference", "benchmark", "lerobot_loss", "all"],
        default="all",
        help="Test to run",
    )
    parser.add_argument("--benchmark_requests", type=int, default=10, help="Number of benchmark requests")
    parser.add_argument("--instruction", type=str, default=None, help="Instruction for inference; omit to use server default")
