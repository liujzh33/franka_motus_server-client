# RC2.0 Dataset Loader for Motus
# Supports RoboChallenge 2.0 data with 4 robot types: ALOHA, ARX5, DOS-W1, UR5
...
# Robot type to action/state dimension mapping
ROBOT_ACTION_DIMS = {
    "ALOHA": 14,    # 7 DOF per arm * 2
    "DOS-W1": 14,   # 7 DOF per arm * 2
    "ARX5": 7,      # 7 DOF single arm
    "UR5": 7,       # 7 DOF single arm
}


class RC2Dataset(data.Dataset):
    """
    Dataset for RC2.0 (RoboChallenge 2.0) data.

    Data structure:
    /cache/.../RC2.0/RoboChallenge2.0_{ROBOT_TYPE}/{task}/{episode_id}/
    ├── videos/episode_{episode_id}.mp4
    ├── qpos/episode_{episode_id}.pt
    ├── metas/episode_{episode_id}.txt
    ...
    def __init__(
        ...
        """
        Initialize RC2.0 dataset.

        Args:
            dataset_dir: Root directory containing {task}/{episode_id}/ subdirectories
            robot_type: Robot type ("ALOHA", "ARX5", "DOS-W1", "UR5")
        ...
        self.action_dim = ROBOT_ACTION_DIMS.get(robot_type, 14)
        ...
        if robot_type not in ROBOT_ACTION_DIMS:
            raise ValueError(f"Unknown robot_type '{robot_type}', must be one of: {list(ROBOT_ACTION_DIMS.keys())}")

        logger.info(f"RC2.0 dataset initialized:")
        logger.info(f"  Robot type: {robot_type}, action_dim: {self.action_dim}")
    ...
    def _load_episodes(self):
        ...
        logger.info("Scanning RC2.0 dataset...")

        all_episodes = []

        # RC2.0 structure: dataset_dir/{task}/{episode_id}/
        # No clean/randomized split - all data in one tree
        ...
        for task_dir in task_dirs:
            ...
            for ep_dir in sorted(task_dir.iterdir()):
                ...
                self.task_to_episodes[task_name].append(episode_data)
        ...
        # Calculate task sampling weights (equal weight)
        num_tasks = len(self.task_to_episodes)
        for task_name in self.task_to_episodes.keys():
            self.task_weights[task_name] = 1.0 / num_tasks

        logger.info(f"RC2.0 dataset: {len(self.task_to_episodes)} tasks, {self.total_episodes} total episodes")
        for task_name, episodes in list(self.task_to_episodes.items())[:5]:
            logger.info(f"  {task_name}: {len(episodes)} episodes")
        if len(self.task_to_episodes) > 5:
            logger.info(f"  ... and {len(self.task_to_episodes) - 5} more tasks")
    ...
    def _load_language_embedding(self, lang_path: str) -> Tuple[torch.Tensor, int]:
        ...
        try:
            ...
            # RC2.0 T5 is a list with 1 element: [seq_len, 4096]
    ...
    def _load_text_instruction(self, meta_path: str, instruction_idx: int = None) -> str:
        ...
        try:
            ...
            if instructions:
                ...
                raise ValueError(f"No instructions found in meta file {meta_path}")
