...
def load_server_components(
    ...
    if stats_path:
        ...
        # If model uses 16-dim (dim16 config), pad stats to 16-dim (same as training
        # lerobot_dataset_dim16.py: _pad_stats_to_16)
        if int(common.get("state_dim", 7)) == 16:
            action_min, action_max = _pad_stats_to_16(action_min, action_max)
            log.info("Padded normalization stats from 7-dim to 16-dim (dim16 config)")
...
def _unify_to_16(arr: np.ndarray) -> np.ndarray:
    """Unify 1D state/action to 16-dim layout (same as training lerobot_dataset_dim16.py).

    7-dim Franka EEF [x,y,z,r,p,y,gripper] ->
    16-dim [x,y,z,r,p,y,0,gripper,0,0,0,0,0,0,0,0]
    """
    ...
    if dim == 16:
        ...
        out = np.concatenate([left, pad, g0, right, pad, g1], axis=1)
        return out.squeeze(0) if arr.shape[0] == 1 else out
    elif dim == 7:
        left = arr[:, :6]; grip = arr[:, 6:7]
        pad1 = np.zeros((arr.shape[0], 1)); right_pad = np.zeros((arr.shape[0], 8))
        out = np.concatenate([left, pad1, grip, right_pad], axis=1)
        return out.squeeze(0) if arr.shape[0] == 1 else out
    elif dim == 8:
        left = arr[:, :7]; grip = arr[:, 7:8]; right_pad = np.zeros((arr.shape[0], 8))
        out = np.concatenate([left, grip, right_pad], axis=1)
        return out.squeeze(0) if arr.shape[0] == 1 else out
    else:
        out = np.pad(arr, ((0, 0), (0, 16 - dim)), mode='constant')
        return out.squeeze(0) if arr.shape[0] == 1 else out


def _pad_stats_to_16(action_min: np.ndarray, action_max: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Pad normalization stats to 16-dim (same as training lerobot_dataset_dim16.py)."""
    min_16 = _unify_to_16(action_min)
    max_16 = _unify_to_16(action_max)
...
def get_state_tensor(state_values: Optional[List[float]], state_dim: int, device: torch.device) -> torch.Tensor:
    ...
    raw_dim = state_np.shape[0]

    # If model expects 16-dim but client sends 7-dim, pad to 16-dim (same as training)
    if state_dim == 16 and raw_dim != 16:
        state_np = _unify_to_16(state_np)
    elif raw_dim != state_dim:
        raise ValueError(f"State length mismatch: expected {state_dim}, got {raw_dim}.")

    # Normalize state to [0,1] using action_min/action_max (same as training:
    # data/lerobot/lerobot_dataset_dim16.py: normalize_actions(state_16d, min_16d, max_16d))
...
def run_model_inference(request: InferenceRequest) -> InferenceResponse:
    ...
    raw_action_dim = None  # will be set if we trim 16→7
    ...
    # If model outputs 16-dim but client expects 7-dim Franka EEF, extract the
    # 7 meaningful dims [x,y,z,r,p,y,gripper] from the 16-dim layout:
    # 16-dim layout: [x,y,z,r,p,y, pad, gripper, 0,0,0,0,0,0,0,0]
    #                  ↑ dim 0-5     ↑ dim 6  ↑ dim 7  ↑ dim 8-15
    action_dim_cfg = int(common["action_dim"])
    if action_dim_cfg == 16 and predicted_actions_cpu.shape[-1] == 16:
        # Extract dims [0:6] (xyz rpy) + dim [7] (gripper) → 7-dim
        pred_7 = torch.cat([
            predicted_actions_cpu[:, :6],   # x,y,z,roll,pitch,yaw
            predicted_actions_cpu[:, 7:8],  # gripper
        ], dim=-1)
        raw_action_dim = predicted_actions_cpu.shape[-1]
        predicted_actions_cpu = pred_7
