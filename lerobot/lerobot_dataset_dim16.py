...
class LeRobotMotusDataset(data.Dataset):
    """Dim16 variant: unifies state/action to 16-dim [left_joints(6)+pad(1)+grip(1)+right_joints(6)+pad(1)+grip(1)]"""

    @staticmethod
    def _unify_to_16(arr):
        """Unify 1D or 2D array to 16-dim layout."""
        ...
        if dim == 16:
            ...
            left = arr[:, :6]; g0 = arr[:, 6:7]; right = arr[:, 7:13]; g1 = arr[:, 13:14]
            pad = np.zeros((arr.shape[0], 1))
            return np.concatenate([left, pad, g0, right, pad, g1], axis=1).squeeze(0) if arr.shape[0] == 1 else np.concatenate([left, pad, g0, right, pad, g1], axis=1)
        elif dim == 7:
            left = arr[:, :6]; grip = arr[:, 6:7]
            pad1 = np.zeros((arr.shape[0], 1)); right_pad = np.zeros((arr.shape[0], 8))
            return np.concatenate([left, pad1, grip, right_pad], axis=1).squeeze(0) if arr.shape[0] == 1 else np.concatenate([left, pad1, grip, right_pad], axis=1)
        elif dim == 8:
            left = arr[:, :7]; grip = arr[:, 7:8]; right_pad = np.zeros((arr.shape[0], 8))
            return np.concatenate([left, grip, right_pad], axis=1).squeeze(0) if arr.shape[0] == 1 else np.concatenate([left, grip, right_pad], axis=1)
        else:
            return np.pad(arr, ((0, 0), (0, 16 - dim)), mode='constant').squeeze(0) if arr.shape[0] == 1 else np.pad(arr, ((0, 0), (0, 16 - dim)), mode='constant')

    @staticmethod
    def _pad_stats_to_16(action_min, action_max):
        """Pad normalization stats to 16-dim using same layout as _unify_to_16."""
        min_16 = LeRobotMotusDataset._unify_to_16(np.array(action_min, dtype=np.float32))
        max_16 = LeRobotMotusDataset._unify_to_16(np.array(action_max, dtype=np.float32))
        return min_16, max_16
    ...
    def __init__(
        ...
        # Custom camera key mapping (for datasets that don't use the standard
        # observation.images.cam_high / cam_left_wrist / cam_right_wrist keys).
        # If provided, these keys are used instead of the defaults for three-camera stitching.
        cam_high_key: Optional[str] = None,
        cam_left_wrist_key: Optional[str] = None,
        cam_right_wrist_key: Optional[str] = None,
        **kwargs
    ):
        super().__init__()
        ...
        if self.task_mode == "single":
            ...
            if isinstance(self.task_name, _OmegaListConfig):
                self.task_name = list(self.task_name)
        ...
        # Custom camera key mapping (e.g., for Franka datasets with global_image/right_image/wrist_image)
        self.cam_high_key = cam_high_key
        self.cam_left_wrist_key = cam_left_wrist_key
        self.cam_right_wrist_key = cam_right_wrist_key

        if cam_high_key is not None and cam_left_wrist_key is not None and cam_right_wrist_key is not None:
            # Use custom camera keys for three-camera stitching
            self.has_concat = False
            self.has_three_cam = all(k in features for k in [cam_high_key, cam_left_wrist_key, cam_right_wrist_key])
            if not self.has_three_cam:
                raise ValueError(
                    f"Custom camera keys not all found in features: "
                    f"{cam_high_key}, {cam_left_wrist_key}, {cam_right_wrist_key}. "
                    f"Available features: {list(features.keys())}"
                )
            logger.info(f"Using custom camera keys: high={cam_high_key}, left_wrist={cam_left_wrist_key}, right_wrist={cam_right_wrist_key}")
        else:
        ...
        # Dim16: pad normalization stats from original dim to 16-dim
        self.action_min, self.action_max = self._pad_stats_to_16(self.action_min, self.action_max)
    ...
    def _t5_cache_file_path_for_task(self, task_idx: int, episode_index: int) -> Optional[Path]:
        """Absolute path for multi-task mode: {task_root}/{t5_folder_name}/episode_XXXXXX.pt"""
        if self.task_mode == "single":
            return self._t5_cache_file_path(episode_index)
        else:
            if hasattr(self, 'lerobot_dataset') and hasattr(self.lerobot_dataset, '_datasets'):
                if task_idx < len(self.lerobot_dataset._datasets):
                    return Path(self.lerobot_dataset._datasets[task_idx].root) / self.t5_folder_name / f"episode_{episode_index:06d}.pt"
            return None
    ...
    def __getitem__(self, idx):
        ...
        task_idx = 0  # default for single mode
        if self.task_mode == "multi":
            ...
            ds_meta = self.lerobot_dataset._datasets[task_idx]
        else:
            ds_meta = self.lerobot_dataset

        # v3.0: use meta.episodes[idx]["dataset_from_index"/"dataset_to_index"]
        # v2.1: use episode_data_index["from"/"to"]
        if hasattr(ds_meta, 'episode_data_index'):
            from_idx_t = ds_meta.episode_data_index["from"][episode_idx]
            to_idx_t = ds_meta.episode_data_index["to"][episode_idx]
        else:
            ep_meta = ds_meta.meta.episodes[episode_idx]
            from_idx_t = ep_meta["dataset_from_index"]
            to_idx_t = ep_meta["dataset_to_index"]
        ...
        if self.has_concat:
            ...
            # Use custom camera keys if provided, otherwise use defaults
            _cam_high = self.cam_high_key or "observation.images.cam_high"
            _cam_left = self.cam_left_wrist_key or "observation.images.cam_left_wrist"
            _cam_right = self.cam_right_wrist_key or "observation.images.cam_right_wrist"
            frames_high = _decode_key(_cam_high)
            frames_left = _decode_key(_cam_left)
            frames_right = _decode_key(_cam_right)
            stitched = []
        ...
        if "observation.state" in item_cond:
        ...
        elif "state" in item_cond:
            initial_state = torch.as_tensor(item_cond["state"]).float()
            ...
            raise KeyError("No state found in item (expected observation.state/state/actions/action)")
        ...
        if all_embeddings is None:
            ...
            if cached is None:
                # v3.0: meta.episodes may not have "t5_embedding_path" field.
                # First check the predictable cache path: {root}/{t5_folder_name}/episode_XXXXXX.pt
                cache_file = self._t5_cache_file_path_for_task(task_idx, ep_index)
                if cache_file is not None and cache_file.exists():
                    emb = torch.load(cache_file, map_location="cpu")
                    if not isinstance(emb, torch.Tensor):
                        emb = torch.tensor(emb)
                    if emb.ndim == 2:
                        emb = emb.unsqueeze(0)
                    ...
                    # Try meta/episodes for t5_embedding_path (v2.1 legacy)
                    # v3.0: meta.episodes is a datasets.Dataset, not a dict; use indexing with bounds check
                    ep_meta = None
                    try:
                        if self.task_mode == 'single':
                            if ep_index < self.lerobot_dataset.meta.episodes.num_rows:
                                ep_meta = self.lerobot_dataset.meta.episodes[ep_index]
                        else:
                            if ep_index < self.lerobot_dataset._datasets[task_idx].meta.episodes.num_rows:
                                ep_meta = self.lerobot_dataset._datasets[task_idx].meta.episodes[ep_index]
                    except Exception:
                        ep_meta = None
                    ...
                    if rel_path is None:
                        if not self.enable_t5_fallback:
                            raise KeyError(
                                "language_embedding not found in item, t5_embedding_path not found in meta, "
                                f"and no cache file at {cache_file}; "
        ...
        # Dim16: unify state and action to 16-dim before normalization
        initial_state_np = self._unify_to_16(initial_state.numpy() if isinstance(initial_state, torch.Tensor) else initial_state)
        action_seq_np = self._unify_to_16(action_sequence.numpy() if isinstance(action_sequence, torch.Tensor) else action_sequence)
        initial_state = torch.from_numpy(initial_state_np).float()
        action_sequence = torch.from_numpy(action_seq_np).float()
