#!/usr/bin/env python3
"""
MotusWanVlmDirectMask Inference API Server for G1 Robot

Provides REST endpoints for G1 humanoid robot inference.
No normalization/denormalization since G1 training didn't use it.
"""
...
log = logging.getLogger("g1_api_server")
...
app = FastAPI(
    title="G1 MotusWanVlmDirectMask Inference API",
    description="REST API for G1 humanoid robot inference",
    version="1.0.0",
)
...
def load_server_components(
    ...
    safetensors_path = Path(ckpt_dir) / "model.safetensors"
    if safetensors_path.exists():
        from safetensors.torch import load_file
        state_dict = load_file(str(safetensors_path), device=str(resolved_device))
        model.load_state_dict(state_dict, strict=False)
        log.info(f"Loaded model from {safetensors_path}")
    else:
        model.load_checkpoint(ckpt_dir, strict=False)
        log.info(f"Loaded model from {ckpt_dir}")
...
def get_state_tensor(state_values: Optional[List[float]], state_dim: int, device: torch.device) -> torch.Tensor:
    ...
    # G1: no normalization, use raw state
...
def run_model_inference(request: InferenceRequest) -> InferenceResponse:
    ...
    # Prepare state (no normalization for G1)
    ...
    # Process outputs (no denormalization for G1)
...
def root() -> Dict[str, Any]:
    return {
        "message": "G1 MotusWanVlmDirectMask Inference API Server",
        "version": "1.0.0",
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="G1 MotusWanVlmDirectMask Inference API Server")
    parser.add_argument("--model_config", required=True, help="Path to YAML config")
    parser.add_argument("--ckpt_dir", required=True, help="Path to checkpoint directory (contains model.safetensors)")
