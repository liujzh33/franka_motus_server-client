...
def get_state_tensor(state_values: Optional[List[float]], state_dim: int, device: torch.device) -> torch.Tensor:
    """Dim16: accepts 7-dim franka state, pads to 16-dim [left6+pad+grip+right_pad8]."""
    if state_values is None:
        return torch.zeros((1, state_dim), dtype=torch.float32, device=device)

    raw_dim = len(state_values)
    if raw_dim == state_dim:
        # Already correct dim
        return torch.tensor(state_values, dtype=torch.float32, device=device).unsqueeze(0)
    elif raw_dim == 7 and state_dim == 16:
        # Pad 7-dim franka to 16-dim: [left_joints(6), pad(1), grip(1), right_pad(8)]
        state_arr = np.array(state_values, dtype=np.float32)
        left = state_arr[:6]
        grip = state_arr[6:7]
        padded = np.concatenate([left, [0.0], grip, np.zeros(8, dtype=np.float32)])
        return torch.from_numpy(padded).float().unsqueeze(0).to(device)
    elif raw_dim == 14 and state_dim == 16:
        # Pad 14-dim to 16-dim: [left6, pad, left_gripper, right6, pad, right_gripper]
        state_arr = np.array(state_values, dtype=np.float32)
        left = state_arr[:6]; g0 = state_arr[6:7]; right = state_arr[7:13]; g1 = state_arr[13:14]
        padded = np.concatenate([left, [0.0], g0, right, [0.0], g1])
        return torch.from_numpy(padded).float().unsqueeze(0).to(device)
    else:
        raise ValueError(f"State length mismatch: expected {state_dim}, got {raw_dim}. Cannot pad.")
...
def run_model_inference(request: InferenceRequest) -> InferenceResponse:
    ...
    # Dim16: Remove padding from 16-dim actions back to original dim
    if predicted_actions_cpu.shape[-1] == 16:
        action_np = predicted_actions_cpu.numpy()
        # Check if this is 7-dim franka (right 8 dims are all pad) or 14-dim robotwin
        # For 7-dim franka: [left6, pad, grip, right_pad8] -> extract [left6, grip] = indices [0:6, 7]
        # For 14-dim robotwin: [left6, pad, left_grip, right6, pad, right_grip] -> indices [0:6, 7:13, 13, 15]
        # Use action_dim from config to decide
        action_dim_cfg = int(common.get("action_dim", 16))
        if action_dim_cfg == 16:
            # Check original dim from state (franka=7, robotwin=14)
            # Default: assume 7-dim franka (extract left arm only)
            action_7d = action_np[:, [0, 1, 2, 3, 4, 5, 7]]  # left6 + grip
            predicted_actions_cpu = torch.from_numpy(action_7d).float()
