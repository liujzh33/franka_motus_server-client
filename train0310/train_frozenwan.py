...
from models.motus_frozenwan import Motus, MotusConfig
...
class UniDiffuserTrainer:
    ...
    def train(self, max_steps: int, resume_from: Optional[str] = None, val_interval: int = 500, reset_scheduler: Optional[bool] = None):
        ...
        while self.global_step < max_steps:
            ...
            #     timing = model._timing
            #     forward_time = t_after_forward - t_data_prep
            #     logger.info(f"=== PROFILING (Step {self.global_step}) ===")
            #     logger.info(f"Forward total: {forward_time*1000:.1f}ms")
            #     # Component breakdown
            #     for key, val in sorted(timing.items()):
            #         logger.info(f"  - {key}: {val*1000:.1f}ms ({val/forward_time*100:.1f}%)")
            #     model._timing = {}
...
def create_model_and_optimizer(config: OmegaConf) -> tuple:
    ...
    model_config = MotusConfig(
        ...
        wan_frozen=getattr(config.model.wan, 'frozen', True),
    )
