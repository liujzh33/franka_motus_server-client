...
from models.motus_wan_vlm_direct_mask_loss_14 import MotusWanVlmDirectMask, MotusWanVlmDirectMaskConfig
    ...
    def log_metrics(self, loss_dict: Dict[str, float], elapsed_time: float):
        ...
        if self.rank == 0:
            logger.info(
                ...
                f"vlm_action={loss_dict.get('vlm_action_loss', 0):.4f}) "
...
def create_model_and_optimizer(config: OmegaConf) -> tuple:
    ...
    model_config = MotusWanVlmDirectMaskConfig(
        ...
        vlm_action_loss_weight=getattr(config.model.loss_weights, 'vlm_action_loss_weight', 0.1),
        enable_vlm_action_loss=getattr(config.model.loss_weights, 'enable_vlm_action_loss', True),
        fast_tokenizer_path=getattr(config.model.loss_weights, 'fast_tokenizer_path', 'physical-intelligence/fast'),
