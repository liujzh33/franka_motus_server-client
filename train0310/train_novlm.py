...
from models.motus_novlm import Motus, MotusConfig
...
def create_model_and_optimizer(config: OmegaConf) -> tuple:
    ...
    # Create Motus config
    # Create Motus config (No VLM version)
    model_config = MotusConfig(
        ...
        # Learnable query settings for Understanding Expert
        num_und_queries=config.model.num_und_queries if hasattr(config.model, 'num_und_queries') else 32,
        und_query_init_scale=config.model.und_query_init_scale if hasattr(config.model, 'und_query_init_scale') else 0.02,
        use_vlm=False,
