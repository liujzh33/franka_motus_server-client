...
class RobotWinTaskDataset(data.Dataset):
    """Dim16 variant: unifies qpos to 16-dim [left_joints(6)+pad(1)+grip(1)+right_joints(6)+pad(1)+grip(1)]
    ...
    def _scan_task_folder(self, task_path: Path) -> List[str]:
        ...
        # Prefer horizontal 3-view layout (videos_threeclip) over original "品" layout
        videos_dir = task_path / "videos_threeclip" if (task_path / "videos_threeclip").exists() else task_path / "videos"
        umt5_dir = task_path / "umt5_wan"
        phase_dir = task_path / "phase_info"
        ...
        for qpos_file in qpos_files:
            ...
            phase_file = phase_dir / f"{episode_name}.json" if phase_dir.exists() else None
            ...
            if video_file.exists() and lang_file.exists():
                ...
                if phase_file is not None and phase_file.exists():
                    episode_data['phase_path'] = str(phase_file)
    ...
    @staticmethod
    def _unify_action_to_16(action: np.ndarray) -> np.ndarray:
        """Unify action to 16-dim: [left_joints(6)+pad(1)+grip(1)+right_joints(6)+pad(1)+grip(1)]"""
        dim = action.shape[1]
        if dim == 16:
            return action
        elif dim == 14:
            ...
            left_joints = action[:, :6]
            grip = action[:, 6:7]
            pad1 = np.zeros((action.shape[0], 1))
            right_pad = np.zeros((action.shape[0], 8))
    ...
    def _load_robot_data(self, qpos_path: str, action_indices: List[int], initial_state_idx: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Load robot position data, unified to 16-dim.
        ...
        # Unify to 16-dim to align with pretrain
        if isinstance(qpos_data, torch.Tensor):
            qpos_np = qpos_data.numpy()
        else:
            qpos_np = np.array(qpos_data, dtype=np.float32)
        qpos_np = self._unify_action_to_16(qpos_np)
        qpos_data = torch.from_numpy(qpos_np)
    ...
    def __getitem__(self, idx: int) -> Optional[Dict[str, Any]]:
        ...
        for _ in range(max_attempts):
            ...
            try:
                ...
                # VLM only sees the high-level task instruction (NOT subtask text)
                vlm_inputs = None
                ...
                # Load phase_info and compute subtask/progress
                subtask_prompt = None
                ...
                if phase_path is not None:
                    try:
                        ...
                        # Compute phase_idx from condition_frame_idx and checkpoints
                        checkpoints = phase_info_dict.get('checkpoints', [])
                        total_steps = phase_info_dict.get('total_steps', 0)
                        phase_idx = 0
                        ...
                        # Get subtask text (use same instruction_idx)
                        if subtasks_list and instruction_idx < len(subtasks_list):
                            task_subtasks = subtasks_list[instruction_idx]
                            if isinstance(task_subtasks, list) and len(task_subtasks) > 0:
                                ...
                                # Remove META_PREFIX from instruction to get clean text
                                ...
                                # Clean subtask text and store only the target (not full prompt)
                                target_text_clean = _clean_subtask_text(subtask_text)
                                subtask_prompt = target_text_clean  # Only the GT target text for logging

                                # Build subtask data for both mot_decoder and native_lm modes
                                if self.vlm_processor is not None:
                                    try:
                                        target_text = _clean_subtask_text(subtask_text)
                                        if target_text:  # Only build if target is non-empty
                                            max_subtask_len = 64
                                            tokenizer = self.vlm_processor.tokenizer

                                            # --- mot_decoder format ---
                                            # Tokenize target text directly (truncation=False first to detect overflow)
                                            target_ids_full = tokenizer(
                                                target_text,
                                                add_special_tokens=False,
                                            )['input_ids']

                                            # Safety check: warn if text exceeds max_length
                                            ...
                                            # BOS + target[:-1] for decoder input, target_ids for labels
                                            ...
                                            # Pad to max_subtask_len
                                            # NOTE: decoder_input_ids has len = len(target_ids) because
                                            # BOS replaces the last token shift. So it's always <= max_subtask_len.
                                            ...
                                            if pad_len > 0:
                                                ...
                                                # This only happens if target_ids had exactly max_subtask_len tokens,
                                                # which is extremely unlikely for short subtask texts.
                                                # Still, truncate safely to avoid shape mismatch.
                                            ...
                                            # Verify: decode labels to confirm FULL text matches (not just first word)
                                            supervised_ids = [tid for tid in decoder_labels if tid != -100]
                                            if supervised_ids:
                                                supervised_text = tokenizer.decode(supervised_ids, skip_special_tokens=True).strip()
                                                # Full text comparison — check that all supervised tokens
                                                # reconstruct the complete target (accounting for tokenizer quirks)
                                                target_decode = tokenizer.decode(target_ids, skip_special_tokens=True).strip()
                                                if supervised_text != target_decode:
                                                    logger.warning(
                                                        f"[SUBTASK LABELS] Full text mismatch! "
                                                        f"target_decode='{target_decode}', "
                                                        f"supervised_text='{supervised_text}', "
                                                        f"original_target='{target_text[:100]}'"
                                                    )
                                                # Also verify first word as extra safety
                                                target_first_word = target_text.split()[0] if target_text.split() else ""
                                                supervised_first_word = supervised_text.split()[0] if supervised_text.split() else ""
                                                if target_first_word.lower() != supervised_first_word.lower():
                                                    logger.warning(
                                                        f"[SUBTASK LABELS] First word mismatch! "
                                                        f"target='{target_first_word}', supervised='{supervised_first_word}', "
                                                        f"target_text='{target_text[:80]}', supervised_text='{supervised_text[:80]}'"
                                                    )

                                            # --- native_lm format (legacy, for backward compatibility) ---
                        ...
                        # Compute progress target: normalized phase position
                ...
                if subtask_lm_inputs is not None:
                    ...
                    # Also add mot_decoder format fields
